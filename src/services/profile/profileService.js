...
export async function uploadProfileImage(userId, imageData, fileName) {
  ...
  if (typeof imageData === 'string') {
    if (imageData.startsWith('data:')) {
      ...
      throw new Error('Image URI provided but base64 is required. Please ensure includeBase64: true in image picker.');
    } else {
    ...
    if (typeof imageData.base64 === 'string' && imageData.base64.startsWith('data:')) {
      imageUri = imageData.base64;
    } else {
      imageUri = `data:${imageData.type || 'image/jpeg'};base64,${imageData.base64}`;
    }
    ...
    console.warn('Uploading image URI without base64. Server may not be able to process it.');
  } else {
    throw new Error('Invalid image data format. Expected base64 string or asset object with base64 property.');
  }
...
export async function saveDatingPreferences(datingPreferences) {
  const headers = await getAuthHeaders();
  return apiClient.post('/profile/dating-preferences', datingPreferences, headers);
}
